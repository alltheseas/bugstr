...
if git rev-parse --git-dir >/dev/null 2>&1; then
    STAGED_CODE=$(git diff --cached --name-only -- 'src/' 2>/dev/null || true)
    STAGED_CHANGELOG=$(git diff --cached --name-only -- 'CHANGELOG.md' 2>/dev/null || true)
...
PII_HITS=$(grep -rEn "$PII_PATTERNS" src/ --include='*.rs' 2>/dev/null | grep -v '// PII-OK' | grep -v 'example\.com' | head -5)
if [ -n "$PII_HITS" ]; then
    echo "$PII_HITS"
